...
from app.db.models import DataSource, DataFetchingTask, JSONValue, S3FileUpload
...
class _TaskProcessor[T: JSONValue](ABC):
    """
...
class _SequentialTaskProcessor[T: JSONValue](_TaskProcessor[T]):
    def __init__(
        self,
        task_id: int,
...
class _BatchTaskProcessor[T: JSONValue](_TaskProcessor[T]):
    def __init__(
        self,
        task_id: int,
...
task_processors: dict[int, _TaskProcessor] = {}
"""
...
async def create_new_task[
    ...
    if batch_size:
        ...
        processor = _BatchTaskProcessor[T](
            task_id=task.id,
        ...
        processor = _SequentialTaskProcessor[T](
            task_id=task.id,
